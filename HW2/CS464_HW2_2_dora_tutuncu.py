...
#uncomment the option below to see all the columns clearly.
...
display(pd.DataFrame(digits_pcomponents)) #transform PC's into database
print("\nReduced Matrix")
display(pd.DataFrame(digits_reduced))
print("\nPrincipal Values Explained Matrix, rows refer to each principle component's PVE:")
display(pd.DataFrame(digits_variance))  
print()

#this for loop is for visualizing each principal component as a 28 by 28 matrix.
for k in range(k_pcomponents) :
    print("\nPrincipal component {} reshaped as a 28x28 matrix: ".format(k+1))
    display(pd.DataFrame(reshaped_pcomponents[k]))
...
#displaying eigenvectors of the dataset
for k in range(k_pcomponents) :
   
    figure()
    imshow(reshaped_pcomponents[k])
    title("Principal component {} shown as a image: ".format(k+1))
...
def Reconstruction(image28x28, k_pcomponent):
    ...
    eigenvalues, pcomponents , reduced = PCA(image28x28, k_pcomponent) # image reconstruction
    recon = np.dot(pcomponents, reduced).T + np.mean(image28x28,axis=0)
...
plt.legend(["houses", "1st order approx."])
plt.show()
...
plt.ylabel('House Price (Million $)')
plt.xlabel('Living Area (Square Feet)')
plt.legend(["houses", "1st order approx.", "2nd order approx."])
plt.show()
